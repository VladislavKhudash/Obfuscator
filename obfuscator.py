...
def obfuscator(file_path, file_extension):
    x0 = lambda data: bytes(n ^ key for n in data)   
    encrypt_code = lambda code: bytes((b + key_code) % 256 for b in code.encode(ENCODINGS[0][0]))
...
def x00(c_code,session_symbols,pattern_main,pattern_another):
    ...
    exec("for n in list(globals().keys()):\\n\\tif not n.startswith('__'):\\n\\t\\tdel globals()[n]");exec(compile(r_code,'<string>','exec'),globals());return compile('exit(0)','<string>','exec')
""".strip().encode(ENCODINGS[0][-1]), level=9))) 
        function_encrypt_encoding = cs_encode(f"__SECRET_KEY__=b'{generator_symbols(randint(2500, 5000), SYMBOLS_HIDDEN, hidden=True)}';__300_000__={function_decrypter};x00B={ENCODINGS[2][-1]};x00C={ENCODINGS[3][-1]};x0=lambda data:bytes(n^{key} for n in data)".encode(ENCODINGS[1][0]), 'uu')
...
def get_python_file():
    ...
    try:
        if not file_extension in ['py', 'pyi', 'pyw']: 
